import type { BayAreaCompanyLogo, BayAreaYearContent } from '@/app/start-goes-bay-area/types';
...
export const bayAreaYearContent: BayAreaYearContent[] = [
  {
    ...
    highlightVisits: [
      ...
      {
        name: 'Etched',
        context: 'South San Francisco',
        logoPath: '/bayarea/logos/etched.svg',
        websiteUrl: 'https://www.etched.com/',
      },
      {
        name: 'LinkedIn Co-founder',
        context: 'Redwood City',
        logoPath: '/bayarea/logos/linkedin.svg',
        websiteUrl: 'https://www.linkedin.com/in/reidhoffman/',
      },
    ],
    heroStats: [
  ...
  },
  {
    ...
    hosts: [
      ...
      },
      {
        ...
        websiteUrl: 'https://www.linkedin.com/in/sebastianthrun/',
      },
      {
      ...
      {
        name: 'The Residency',
        logoPath: '/bayarea/logos/the-residency.svg',
...
export const bayAreaVisitCompanyLogos: BayAreaCompanyLogo[] = (() => {
  ...
  for (const year of bayAreaYearContent) {
    ...
    for (const logo of yearLogos) {
      ...
      const shouldPreferNewName =
        existing.name.toLowerCase().includes('visit') && !logo.name.toLowerCase().includes('visit');

      // Keep whichever entry has richer metadata when duplicates exist across years.
      if (
        shouldPreferNewName ||
        (!existing.websiteUrl && logo.websiteUrl) ||
        (!existing.logoTheme && logo.logoTheme)
      ) {
